...
int force_treeevaluate_shortrange(int target, int mode)
{
  ...
  double r2, dx, dy, dz, pmass, mass, r, fac, u, h, h_inv, h3_inv;
  double acc_x, acc_y, acc_z, pos_x, pos_y, pos_z, aold;
  double eff_dist;
  ...
  if(ptype == 0 && pmass*1.0e10/All.HubbleParam < All.RefinementMass || ptype == 1 && pmass*1.0e10/All.HubbleParam < (All.Omega0/All.OmegaBaryon-1.0)*All.RefinementMass)
    {
      rcut = All.Rcut[1];
      asmth = All.Asmth[1];
    }
...
void force_treeevaluate_potential_shortrange(int target, int mode)
{
  ...
  double r2, dx, dy, dz, pmass, mass, r, u, h, h_inv, wp;
  double pot, pos_x, pos_y, pos_z, aold;
