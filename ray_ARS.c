...
void raytrace_TG(double dt_raytrace)
  {
    ...
    double star_mass, n_max= 1.e5, n_cut = 1.e12;
    ...
    if(All.ray_flag_sun == 0)
      ...
      { 
        ...
        ray.Q_H_ion =  8.626008e+39;
        ray.Q_He_ion = 1.11629e+09;
                              ...
                              if(ray.r[k] >= sim_r_lower && ray.r[k] <= sim_r_upper)
                                {
                                  ...
                                  ray.nh_local[i][j][k] += nh_here*nh_here*nh_here;
                                  if(k == 0)
                                    ray.NH2_local[i][j][k] +=  nh_here*nh_here*nh_here*SphP[n].TracAbund[IH2]*(ray.r[k] - 0);
              ...
              if(d_N > N_photons_per_ray)
                {
                  if(ray.He_front[i][j] > r_k_lower && ray.He_front[i][j] < r_k_upper)
                    ...
                    ray.He_front[i][j] = r_k_lower + N_photons_per_ray / d_N * dr_k; 
                }
