...
void ngb_treebuild(void)
{
  ...
  if(ThisTask == 0)
    printf("Ngb-Tree construction finished \n");
}

/*! This function returns neighbours that are sinks with distance <= hsml and returns them in
 *  Ngblist. Actually, particles in a box of half side length hsml are
 *  returned, i.e. the reduction to a sphere still needs to be done in the
 *  calling routine.
 */
int ngb_treefind_variable_sink(FLOAT searchcenter[3], FLOAT hsml, int *startnode)
{
  int k, numngb;
  int no, p;
  struct NODE *this;
