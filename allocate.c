...
void allocate_commbuffers(void)
{
  ...
  /*SINK*/
  All.BunchSizeSink= (All.BufferSize*1024*1024)/(sizeof(struct sinkdata_in) + 2*sizeof(struct sinkdata_out));

  SinkDataIn= (struct sinkdata_in *)CommBuffer;
  SinkDataResult=(struct sinkdata_out *)(SinkDataIn + All.BunchSizeSink);
  SinkDataPartialResult=SinkDataResult+All.BunchSizeSink;

  NgbsDataIn= (struct ngbsdata_in *)CommBuffer;
   /*SINK*/
  All.BunchSizeDomain =
