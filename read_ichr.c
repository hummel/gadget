...
void empty_read_buffer(enum iofields blocknr, int offset, int pc, int type)
{
  ...
  switch (blocknr)
    ...
    case IO_SINK:
      //printf("reading sink values! \n");
      for(n=0; n<pc; n++)
        SphP[offset + n].sink = *fp++;
      break;
...
void read_file(char *fname, int readTask, int lastTask)
{
  ...
  int type;
  char label[4];
  int nstart, bytes_per_blockelement, npart, nextblock, typelist[numtype];
...
*/
  ...
  if(ThisTask == readTask)
    {
      for(i = 0, n_in_file = 0; i < numtype; i++)
  ...
  for(type = 0, nall = 0; type < numtype; type++)
    {
      ...
      if(blockpresent(blocknr))
	{
	  ...
	  if(npart > 0)
	      ...
	      if(ThisTask == readTask)
		{
	      ...
	      for(type = 0, offset = 0; type < numtype; type++)
		{
		  n_in_file = header.npart[type];
#ifdef HAVE_HDF5
		  pcsum = 0;
