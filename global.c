...
void compute_global_quantities_of_system(void)
{
  ...
  for(i = 0; i < NumPart; i++)
...
#else /* ISOTHERM_EQS */
#ifdef POLYTROPE
          egyspec = get_energy(SphP[i].Density) / SphP[i].Density;
#else /* POLYTROPE */
#ifdef CHEMCOOL
	  gamm1 = SphP[i].Gamma - 1.0;
	  egyspec = entr / gamm1 * pow(SphP[i].Density / a3, gamm1);
#else /* CHEMCOOL */
      ...
      for(j = 0; j < 3; j++)
	{
      ...
      if (P[i].Type == 0) {
        for(j = 0; j < TRAC_NUM; j++) {
	  sys.MolAbund[j] += P[i].Mass * SphP[i].TracAbund[j];
	}
      }
#endif
    }
...
#ifdef CHEMCOOL
  MPI_Reduce(&sys.MolAbund[0], &SysState.MolAbund[0], TRAC_NUM, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
#endif
...
#ifdef CHEMCOOL
      for(j=0; j<TRAC_NUM; j++) {
	SysState.MolAbund[j] /= SysState.MassComp[0];
      }
#endif
    }
